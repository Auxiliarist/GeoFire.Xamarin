...
namespace GeoFire.Xamarin.Android.Core
{
    public class GeoHashQuery
    {
        ...
        public static GeoHashQuery QueryForGeoHash(GeoHash geohash, int bits)
        {
            ...
            string baseString = hash.Substring(0, hash.Length - 1);
            int lastValue = Base32Utils.Base32CharToValue(hash.ElementAt(hash.Length - 1));
            int significantBits = bits - (baseString.Length * Base32Utils.BITS_PER_BASE32_CHAR);
            ...
            string startHash = baseString + Base32Utils.ValueToBase32Char(startValue);
            string endHash;

            if (endValue > 31)
                endHash = baseString + "~";
            else
                endHash = baseString + Base32Utils.ValueToBase32Char(endValue);
        ...
        {
            int startCompare = other.StartValue.CompareTo(StartValue);
        ...
        public override bool Equals(object obj)
        {
            ...
            GeoHashQuery that = (GeoHashQuery)obj;
        ...
        public static class Utils
        {
            ...
            public static int BitsForBoundingBox(GeoLocation location, double size)
            {
                ...
                int bitsLatitude = (int)Math.Floor(BitsLatitude(size)) * 2;
                int bitsLongitudeNorth = (int)Math.Floor(BitsLongitude(size, latitudeNorth)) * 2 - 1;
                int bitsLongitudeSouth = (int)Math.Floor(BitsLongitude(size, latitudeSouth)) * 2 - 1;
